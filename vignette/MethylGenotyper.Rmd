...
- __DS:__ $G = P_1 + 2P_2$
- __AF:__ $q = \frac{\sum G}{2N}$, where $N$ is the sample size
- __$R^2$:__ $R^2 = \frac{\sigma^2}{2q(1-q)}$, where $\sigma^2$ is the variance of dosage genotypes
...
This package has the following dependencies: `minfi`, `tidyverse`, `foreach`, `doParallel`, `HardyWeinberg`, `multimode`, `rlist`, `stats4`.
...
For samples from a mixed population, we recommended to recalibrate the genotype calls based on individual-specific AFs. This process can be done in three steps: 1) Project the study samples to reference ancestry space; 2) Estimate individual-specific AFs; 3) Re-calculate the posterior genotype probabilities based on the individual-specific AFs. Here are the detailed description of the three steps.

* __step 1:__ Principal Components Analyses (PCA) are conducted in 1KGP individuals and a combination of 1KGP individuals and each study sample. Then, Projection Procrustes analyses are conducted to project each study sample to reference ancestry space. This step was implemented by the TRACE software ([Wang et al. Nat Genet 2014](https://www.nature.com/articles/ng.2924), [Wang et al. Am J Hum Genet 2015](http://dx.doi.org/10.1016/j.ajhg.2015.04.018)).

* __Step 2:__ For each SNP, we model genotypes of the reference individuals as a linear function of top four PCs ($v$): $G\sim\beta_0+\beta_1v_1+\beta_2v_2+\beta_3v_3+\beta_4v_4$. Then, the AF ($q$) for each SNP can be obtained by: $\hat{q}=\frac{1}{2}(\hat{\beta_0}+\hat{\beta_1}\hat{v_1}+\hat{\beta_2}\hat{v_2}+\hat{\beta_3}\hat{v_3}+\hat{\beta_4}\hat{v_4})$. ([Dou et al. PLoS Genet 2017](https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1007021))

* __Step 3:__ This step is based on the Bayesian approach, which can be implemented by re-run the `get_GP` function.
...
pc <- projection(dosage, plotPCA=TRUE, cpu=10)

# Recalibrate genotypes for SNP probes, Type I CCS probes, and Type II CCS probes
genotype_snp_recal <- recal_Geno(genotype_snp, type="snp_probe", refPC=pc$refPC, studyPC=pc$studyPC)
genotype_typeI_recal <- recal_Geno(genotype_snp, type="typeI_ccs_probe", refPC=pc$refPC, studyPC=pc$studyPC)
genotype_typeII_recal <- recal_Geno(genotype_snp, type="typeII_ccs_probe", refPC=pc$refPC, studyPC=pc$studyPC)
